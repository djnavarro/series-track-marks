...
if(scheme == 1) {pl <- "grDevices::Purples"; mesh <- .05}
if(scheme == 2) {pal <- hcl.colors(100, palette = "PuRd"); bg <- "grey10"; mesh <- .01; iter <- 100000}
if(scheme == 3) {pl <- "grDevices::Oranges"; ex <- -.0004; rd <- .001; layers <- 10; f <- 1.5; iter <- 50000}
if(scheme == 4) {f <- .9; pf <- jasmines::palette_named("blood"); ex <- -.0006; rd <- .002; pal <- pf(256); bg <- "black"; mesh <- .1; iter <- 50000}
if(scheme == 5) {pl <- "scico::bilbao"; bg <- "grey10"; sifttype <- "type2"; iter <- 100000; mesh <- .01}
if(scheme == 6) {f <- 1; pal <- hcl.colors(256, palette = "Blues"); iter <- 50000; mesh <- .015}
if(scheme == 7) {pal <- scico::scico(4, palette = "grayC"); iter <- 100000} # <- slooooow
if(scheme == 8) {pl <- "scico::bamako"; ex <- -.0004; rd <- .001; iter <- 50000; f <- .5}
...
sift <- function(grain = .025, type = "filtered") {
  function(data) {
    if(type == "filtered") {
      return(
        data %>% 
          group_by(group) %>%
          filter((max(x)-min(x)) < grain, (max(y) - min(y)) < grain) %>%
          ungroup()
      )
    }
    if(type == "type2") {
...
p <- ggplot(
  ...
  geom_voronoise(perturb = sift(mesh, sifttype), max.radius = .05, radius = rd, expand = ex) +
  scale_fill_identity() + 
