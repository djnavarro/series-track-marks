...
if(scheme == 2) {pal <- hcl.colors(100, palette = "PuRd"); bg <- "grey10"; flip <- FALSE}
if(scheme == 3) {pl <- "grDevices::Oranges"; layers <- 50; f <- 1.5}
if(scheme == 4) {f <- 2; layers <- 1; pf <- jasmines::palette_named("blood"); pal <- pf(256); bg <- "black"; mesh <- .05}
if(scheme == 5) {pl <- "scico::bilbao"; bg <- "grey10"}
if(scheme == 6) {f <- 1; pal <- hcl.colors(3, palette = "Blues"); iter <- 50000; mesh <- .015}
if(scheme == 7) {pal <- scico::scico(4, palette = "grayC"); iter <- 100000} # <- slooooow
if(scheme == 8) {pl <- "scico::bamako"; ex <- -.0004; rd <- .001; iter <- 50000; mesh <- .02}
...
p <- ggplot(
  ...
  geom_voronoise(perturb = sift(mesh), max.radius = .05, radius = rd, expand = ex) +
  scale_fill_identity() + 
